...
const PersonView = ({name, role, img, email, twitter, instagram, link}) => {
  return (
        ...
        <div className='float-right absolute xs:relative w-full xs:w-[64%] sm:w-[65%] m-l:w-2/3 lg:w-[70%] flex justify-center h-full duration-200 z-20'>
          <div className='h-auto my-auto pb-2 pr-4 w-full'>
            <div className='font-extrabold w-full leading-5 3xs:leading-6 text-xl 3xs:text-2xl md:text-3xl md:leading-7 text-white block duration-200'>
              <p className='float-right xs:float-left'>{name}</p>
            </div>
            <div className='font-extrabold w-full text-sm 3xs:text-lg text-white block'>
              <p className='float-right xs:float-left'>{role}</p>
            </div>
            <div className='inline-block w-full xs:float-left float-right'>

...
const position_view = ({ topic, image, link, lp, key }) => {
  return (
    <Link key={key} className={`relative  w-auto h-auto group aspect-4/3 3xs:aspect-video xs:aspect-4/3 ${!lp ? 'md:aspect-video' : 'last-of-type:md:hidden'} rounded-sm shadow-md shadow-zinc-300 overflow-hidden`} to={link}>

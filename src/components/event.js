...
const Event = ({Day, Month, EventName, Description}) => {
    ...
    return (
    <div onClick={e => {e.preventDefault(); if((Description !== null && Description !== "" && Description !== undefined)){ setShowMore(!showMore)}}}  className={`min-h-[calc(100vh-)] mb-6 w-full xs:w-11/12 sm:w-9/10 md:w-[85%] mx-auto ${(Description !== null && Description !== "" && Description !== undefined) && 'cursor-pointer'} skew-y-2 bg-gradient-to-r from-magenta to-[#5800FF] text-white duration-200`}>
